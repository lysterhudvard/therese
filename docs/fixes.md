...
## 48. Cinematic Hero Logo Handoff, Hydration Drift & Scroll-Deferred Swap
- **Symptom:** During the cinematic header animation sequence, returning users saw a blank spot in the top-left corner before the logo loaded. On first-time visits, the logo completed its CSS movement to the corner but then faded out completely, disappearing and then visually "popping" back in a fraction of a second later (a duplicate ghost logo effect).
- **Root Cause:**
  1. **Hydration Lag & Conditional Render:** The React Nav logo was set behind a client-side conditional render state (`logoSwapped`). During Astro's static site generation (and during the initial page load before React hydration completed), React rendered an empty spacer. Because repeat visits bypassed the intro and instantly hid the Astro logo, this resulted in an empty gap until React finished loading and set `logoSwapped = true`.
  2. **Timer Asynchrony:** First-time visits relied on React's `useEffect` running a `setTimeout` to guess when the Astro CSS animation finished and swap the visibility. Because React hydration takes variable time depending on client hardware, the timer drifted. The Astro logo finished its CSS lifecycle and faded out before React mounted the new logo.
  3. **Visual Ghosting:** Framer Motion's `layoutId` was left on the React logo button. When it mounted, it tried to run a long 1.6s morph transition without an origin element, resulting in double-text ghosting.
- **Resolution:**
  - **Scroll-Deferred Logo Swap:** The Astro logo is now a permanent native HTML link (`<a href="#top">`) that does not fade out. When the intro finishes, the Astro logo remains in the top-left indefinitely. It only swaps to the React Nav logo once the user scrolls down (crossing `scrollY > 100` or clicking a navigation link), hiding the subpixel snap/jump in active scrolling motion.
...
## 53. Deconstruct React Page Island into Fine-Grained Astro Islands (Eliminating Giant JS Hydration Chains)
- **Symptom:** PageSpeed Insights flagged a long chain of critical request JS files (`index.js -> react-dom.js -> client.js -> index.tsx` etc.). This "Giant React Island" delayed TBT and hydration, keeping the PageSpeed score around 85/100 instead of moving to the 90+ green zone.
- **Root Cause:** The main page was structured as a single massive `<Page client:idle />` React component, compiling all sections (Biography, Portfolio, Showreels, Credits, Voice, Contact, Footer) and their layout code into one single large JavaScript bundle.
- **Resolution:**
  - **Deconstructed the Giant Island:** Broke the monolithic React component into independent, granular islands: `<Nav client:idle />`, `<Spotlight client:idle />`, `<Biography client:visible />`, `<Portfolio client:visible />`, `<Showreels client:visible />`, `<Credits client:visible />`, `<Voice client:visible />`, `<Contact client:visible />`, `<Footer client:visible />`, and `<CommentaryPlayer client:idle />`.
  - **Intersection-driven Cinematic Scope Bars:** Extracted the anamorphic scope bars from React state and Framer Motion into static HTML/CSS inside `index.astro` and `styles.css`. Animated their entry and exit using a highly optimized, native browser `IntersectionObserver` that toggles a class on the `<html>` element.
  - **Shared Store & Event Bus for Commentary:** Refactored the callback-based communication between `Credits` and the audio `CommentaryPlayer` to use a global, reactive vanilla store pattern inside `use-t.tsx`. This enables state-synchronized interaction across distinct React islands without any React parent tree dependency.
  - **Global Translation & Hydration Sync:** Created a global reactive language store in `use-t.tsx` that synchronizes SV/EN toggle updates across the islands. Leveraged Astro's server-side frontmatter to parse Supabase CMS database fields (such as biography sections, image credits, and review quotes) and seed the global state for SSR rendering, and serialized it onto `window.__INITIAL_DB_DATA__` for automatic, client-side, zero-mismatch hydration.

## 54. Zero-Height Collapsed Hydration Observer & Icon Library De-bloat
- **Symptom:** PageSpeed score stayed at 87/100 instead of 90+ due to `Biography.js` and other below-the-fold component JS scripts downloading instantly on load despite `client:visible`. In addition, `CommentaryPlayer.js` downloaded heavy icon scripts (e.g. `createLucideIcon.js`, `play.js`, `volume-2.js`), blocking page threads.
- **Root Cause:**
  1. **Zero-Height Hydration Trap:** Prior to React hydration, components set to `client:visible` (like Biography, Portfolio, Credits) have a raw HTML render height of `0px` because their `<astro-island>` wrappers default to `display: contents`. This collapses them to the top of the viewport. Astro's visibility observer detects them at the top of the page immediately, triggering eager hydration and downloading the JS chunks at ~1.3s.
  2. **Lucide Icons Bundler Bloat:** `CommentaryPlayer` imported visual cues from `lucide-react` directly, compiling the icons framework libraries into the player bundle.
- **Resolution:**
  - **CSS Intrinsic Sizing & Explicit Wrappers:** Wrapped each React island component inside `index.astro` with an explicitly sized native container (`<div style="min-height: 80vh;">`). This absolutely guarantees that the `<astro-island>` tags are stacked far below the initial viewport fold prior to React hydration, completely closing the zero-height eager hydration loop.
  - **Deferred Commentary Component Loading:** Swapped `<CommentaryPlayer client:idle />` to `<CommentaryPlayer client:visible />` in `index.astro` so its JavaScript assets are not loaded until the viewport approaches the Credits section.
  - **Zero-Dependency SVG Icons:** Removed the `lucide-react` import entirely inside `CommentaryPlayer.tsx` and replaced all of its vector elements (`X`, `Play`, `Pause`, `VolumeX`, `Volume2`) with raw inline SVGs. This completely strips the Lucide runtime bundle out of the client JS.

## 55. Eliminating Framer Motion Rendering Bottlenecks on Mobile
- **Symptom:** The initial page load (specifically on mobile devices) was heavily blocked for almost 2 seconds by Framer Motion libraries (`AnimatePresence.js`, `use-scroll.js`, `use-transform.js`), preventing the page from reaching the 90+ green zone.
- **Root Cause:** The `<Nav />` and `<Spotlight />` components sat at the top of the viewport and were eagerly loading on `client:idle`. The `<Nav />` component imported `framer-motion` for simple mobile menu toggles and layout transitions. The `<Spotlight />` component (a mouse hover effect) was uselessly hydrating on touch devices.
- **Resolution:**
  - **Native React State & CSS in `<Nav />`:** Completely removed the `framer-motion` import from `Nav.tsx`. Replaced `<motion.button>`, `<motion.div>`, and `<AnimatePresence>` with standard HTML elements (`<button>`, `<div>`), utilizing native React state and CSS `transition` properties for all animations (including the hamburger menu, language switcher fade, and menu slide-down).
  - **Media Query Island Deactivation:** Changed the `<Spotlight />` island directive in `index.astro` from `client:idle` to `client:media="(min-width: 1024px)"`. This prevents Astro from ever downloading or hydrating the Spotlight cursor effect JS on mobile and tablet devices, instantly shedding massive JS weight from the critical path.
## 56. Framework Engine Substitution (React to Preact)
- **Symptom:** Lighthouse reported approximately ~50 KiB of unused JavaScript overhead coming directly from `client.js` and `proxy.js` (the core Astro hydration and React DOM runtime files).
- **Root Cause:** The default React integration (`@astrojs/react`) bundles the full React DOM framework, which is notoriously heavy. Because Lighthouse runs audits without simulating user interactivity, the majority of the React state/event handlers go completely unused during the paint trace, resulting in an "Unused JavaScript" penalty.
- **Resolution:**
...
## 80. TypeScript / Preact JSX Compatibility Errors in Unused UI Components

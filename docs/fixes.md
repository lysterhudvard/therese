...
- **Root Cause:**
  1. **Zero-Height Hydration Trap:** Prior to React hydration, components set to `client:visible` (like Biography, Portfolio, Credits) have a raw HTML render height of `0px` because they have no placeholder styling. This collapses them to the top of the viewport. Astro's visibility observer detects them at the top of the page immediately, triggering eager hydration and downloading the JS chunks at ~1.3s.
...
- **Resolution:**
  - **CSS Intrinsic Sizing & Min Heights:** Added `content-visibility: auto`, `contain-intrinsic-size: 800px`, and a fallback `min-height: 80vh` to sections (`#bio`, `#portfolio`, `#showreels`, `#credits`, `#voice`, `#contact`) in `src/styles.css`. This ensures the browser pre-allocates layout boxes before hydration, pushing below-the-fold components far outside the initial fold, preventing early hydration trigger.
